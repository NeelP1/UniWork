...
for(i in 1:length(deptSummary$department_name)){
  ...
  for(j in 1:length(deptItems[,1])){
    ...
    catalogueSum[j,1] <- mean(item$sales_unit)
  }
...
for(i in 1:length(deptSummary$department_name)){
  ...
  for(j in 1:length(deptItemsb[,1])){
    ...
    instoreSum[j,1] <- mean(item$sales_unit)
  }
...
for(i in 1:length(deptSummary$department_name)){
  ...
  for(j in 1:length(deptItemsc[,1])){
    ...
    nopromotionSum[j,1] <- mean(item$sales_unit)
  }
...
pie(deptSummary[,2], main = "Average mean of unit sales (Catalogue Promotion)",
    labels=deptSummary[,2], cex=0.7, col=colours)

legend("topright", deptNames, fill=colours, cex=0.8)

#instore promotions
pie(deptSummary[,3], main = "Average mean of unit sales (In-store Promotion)",
    labels=deptSummary[,3], cex=0.7, col=colours)

legend("topright", deptNames, fill=colours, cex=0.8)

#no promotion
pie(deptSummary[,4], main = "Average mean of unit sales (No Promotion)",
...
write.xlsx(x = deptSummary, file = "dept_summary_mean.xlsx", sheetName = "summary", row.names = FALSE)
